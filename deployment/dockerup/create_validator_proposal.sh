...
function generate() {
    ...
    echo "docker exec mocad-validator-0 mocad tx bank send validator0 $VALIDATOR_ADDR 10000000000000000000000000amoca --keyring-backend test --node http://localhost:26657 -y --fees 6000000amoca" >>${SCRIPT_DIR}/send_tokens.sh
    echo "docker exec mocad-validator-0 mocad tx bank send validator0 $DELEGATOR_ADDR 10000000000000000000000000amoca --keyring-backend test --node http://localhost:26657 -y --fees 6000000amoca" >>${SCRIPT_DIR}/send_tokens.sh
}
...
function query_proposal() {
    ...
    $MOCAD_CMD q tx $tx_hash --home /app --output json >${SCRIPT_DIR}/tx.json
    ...
    echo "docker exec mocad-validator-0curl -s http://127.0.0.1:1317/cosmos/gov/v1/proposals/$proposal_id | jq"
}
...
function vote() {
    ...
    vote_id=$2
    for ((i = 0; i < ${size}; i++)); do
        docker exec mocad-validator-${i} mocad tx gov vote $vote_id yes --from=validator$i --chain-id="moca_5151-1" --keyring-backend=test --gas-prices=10000amoca -y
    done
}

function show_validator() {
    docker exec mocad-validator-0 curl -s http://localhost:1317/cosmos/staking/v1beta1/validators | jq '.pagination.total'
}
