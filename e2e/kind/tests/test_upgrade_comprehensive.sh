...
#   - pre-upgrade transaction functions
#   - post-upgrade transaction functions
...
#   2. Implement: <name>_pre_upgrade, <name>_post_upgrade, <name>_register_tests
...
_PRE_UPGRADE_FNS=()
_POST_UPGRADE_FNS=()
_TEST_FNS=()        # array of "description|function_name" pairs

register_pre_upgrade()  { _PRE_UPGRADE_FNS+=("$1"); }
register_post_upgrade() { _POST_UPGRADE_FNS+=("$1"); }
register_test()         { _TEST_FNS+=("$1|$2"); }
...
if [ ${#_PRE_UPGRADE_FNS[@]} -eq 0 ] && [ ${#_TEST_FNS[@]} -eq 0 ]; then
...
# If a release image is specified, pull and load it for the upgrade target
...
# ══════════════════════════════════════════════════════════════════════════════
# PRE-UPGRADE: run all module pre-upgrade functions
# ══════════════════════════════════════════════════════════════════════════════
PRE_HEIGHT=$(get_block_height "http://localhost:26657")
log_info "=== Pre-upgrade phase (height=${PRE_HEIGHT}) ==="

for fn in "${_PRE_UPGRADE_FNS[@]}"; do
    log_info "Running pre-upgrade: ${fn}"
    "$fn"
done
...
# ══════════════════════════════════════════════════════════════════════════════
# POST-UPGRADE: run all module post-upgrade functions
# ══════════════════════════════════════════════════════════════════════════════
log_info "=== Post-upgrade phase ==="

for fn in "${_POST_UPGRADE_FNS[@]}"; do
    log_info "Running post-upgrade: ${fn}"
    "$fn"
done

# ══════════════════════════════════════════════════════════════════════════════
# VERIFICATION: common tests + module-registered tests
# ══════════════════════════════════════════════════════════════════════════════
...
# Module-registered tests
for entry in "${_TEST_FNS[@]}"; do
    desc="${entry%%|*}"
    func="${entry##*|}"
