...
# Create directories
create_directories() {
  if [ ! -d "$project_dir/precompiles/contracts/artifacts" ]; then
    mkdir -p "$project_dir/precompiles/contracts/artifacts"
  fi
}
...
generate_abigen() {
  ...
  for contract in "${contracts[@]}"; do
    ...
    $abigen_path --abi "$project_dir/precompiles/contracts/artifacts/${contract}.abi" \
      --bin "$project_dir/precompiles/contracts/artifacts/${contract}.bin" \
      --type "${contract}" --pkg "${pkg}" \
      --out "$project_dir/precompiles/${pkg}/${contract}.go"
  done
}

# Clean up generated files
cleanup() {
  rm -rf "$project_dir/precompiles/contracts"
}
