...
func (tcq *TransactionCacheQueue) updateMetrics() {
	// Compute values before acquiring metrics lock to avoid deadlock
	// (getTotalCachedCount acquires tcq.mu, which could deadlock with
	// enforceMemoryLimits holding tcq.mu and calling updateMemoryMetrics
	// which acquires metrics.mu)
	totalCached := int64(tcq.getTotalCachedCount())

	tcq.mu.RLock()
	accountCount := int64(len(tcq.accounts))
	tcq.mu.RUnlock()

	tcq.metrics.mu.Lock()
	defer tcq.metrics.mu.Unlock()

	tcq.metrics.CachedTxCount = totalCached
	tcq.metrics.AccountsWithCache = accountCount
...
// Returns cached values; metrics are kept up-to-date by background workers
// (enforceMemoryLimits, cleanupExpiredTransactions) to avoid lock contention.
...
func (tcq *TransactionCacheQueue) removeOldestTransactionsFromAccount(addr common.Address, count int) {
	...
	// Remove oldest transactions (directly manipulate internals since we already hold queue.mu)
	removed := 0
	for _, tx := range txs {
		if removed >= count {
			break
		...
		delete(queue.transactions, tx.nonce)
		queue.sortedNonces.Remove(tx.nonce)
		removed++
