...
// SimulateMsgAttest's body is a `// TODO` stub: it never dereferences its
...
func TestSimulateMsgAttest(t *testing.T) {
	r := rand.New(rand.NewSource(1)) //nolint:gosec
	accs := simtypes.RandomAccounts(r, 3)

	op := simulation.SimulateMsgAttest(nil, nil, keeper.Keeper{})
