...
func (s *TestSuite) TestCompleteMigrateBucket_AcceptsOwnFamily() {
	...
	s.spKeeper.EXPECT().MustGetStorageProvider(gomock.Any(), gomock.Any()).
		DoAndReturn(func(_ sdk.Context, id uint32) *sptypes.StorageProvider {
			return &sptypes.StorageProvider{Id: id}
		}).AnyTimes()
