...
func (s *BurnTestSuite) orphanCommonMocks() {
	...
	s.evmKeeper.EXPECT().EstimateGas(gomock.Any(), gomock.Any()).
		Return(&evmtypes.EstimateGasResponse{Gas: 100000}, nil).AnyTimes()
	s.evmKeeper.EXPECT().ApplyMessage(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
