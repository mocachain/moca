...
func CmdToggleSPAsDelegatedAgent() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "toggle-sp-as-delegated-agent [bucket-name] --privatekey xxx",
		Short: "Toggle whether SP is disabled as delegated upload agent for a bucket",
		Args:  cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			argPrivateKey, err := cmd.Flags().GetString(FlagPrivateKey)
			if err != nil {
				return err
			}
			argBucketName := args[0]
			...
			txRsp, err := session.ToggleSPAsDelegatedAgent(argBucketName)
			if err != nil {
				return err
			}
			...
			_, err = sdkclient.WaitForEvmTx(context.Background(), clientCtx.EvmClient, gnfdCli, txRsp.Hash())
			if err != nil {
				return fmt.Errorf("failed to toggle SP as delegated agent.%v", err.Error())
			}
			return clientCtx.PrintObjectLegacy(txRsp.Hash().String())
		},
	}

	cmd.Flags().String(FlagPrivateKey, "", "The privatekey of account to toggle SP as delegated agent")
	flags.AddTxFlagsToCmd(cmd)
