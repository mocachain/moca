...
func (suite *KeeperTestSuite) TestMintAndAllocateInflation() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			// Note: incentives are now sent to community pool instead of incentives module
			// Check community pool balance instead
			communityPoolAddr := suite.app.AccountKeeper.GetModuleAddress("distribution")
			...
			feePool, _ := suite.app.DistrKeeper.FeePool.Get(suite.ctx)
			balanceCommunityPool := feePool.CommunityPool
...
func (suite *KeeperTestSuite) TestGetCirculatingSupplyAndInflationRate() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {},
			func() sdk.Dec { d, _ := sdk.NewDecFromStr("51.562500000000000000"); return d }(),
		},
		{
			"low supply",
			sdk.TokensFromConsensusPower(400_000_000, evmostypes.PowerReduction).Sub(bondedAmt),
			func() {},
			func() sdk.Dec { d, _ := sdk.NewDecFromStr("154.687500000000000000"); return d }(),
		},
		{
...
func (suite *KeeperTestSuite) TestBondedRatio() {
	testCases := []struct {
		...
		},
		{
			...
			false,
			func() {},
			func() sdk.Dec { d, _ := sdk.NewDecFromStr("0.999900009999000099"); return d }(),
		},
	}
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
