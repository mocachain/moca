...
var (
	...
	DefaultExponentialCalculation = ExponentialCalculation{
		A:             math.LegacyNewDecFromInt(math.NewInt(300_000_000)),
		R:             math.LegacyNewDecWithPrec(50, 2), // 50%
		C:             math.LegacyNewDecFromInt(math.NewInt(9_375_000)),
		BondingTarget: math.LegacyNewDecWithPrec(66, 2), // 66%
		MaxVariance:   math.LegacyNewDecFromInt(math.ZeroInt()),             // 0%
	}
...
func validateExponentialCalculation(i interface{}) error {
	...
	if v.R.GT(math.LegacyNewDecFromInt(math.NewInt(1))) {
	...
	if v.BondingTarget.GT(math.LegacyNewDecFromInt(math.NewInt(1))) {
...
func validateInflationDistribution(i interface{}) error {
	...
	if !totalProportions.Equal(math.LegacyNewDecFromInt(math.NewInt(1))) {
