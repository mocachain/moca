...
- (app) [#390](https://github.com/mocachain/moca/pull/390) Let a hardfork configured in `app.toml` take precedence over an upgrade plan that is only pending, instead of panicking in `BeginBlock`. Governance can schedule a plan for a height an operator has also configured, and the panic left a node with nothing to recover to. The configured hardfork now replaces a pending or stale plan — `ScheduleUpgrade` already clears the old plan and its IBC state — and a plan that cannot be read is left untouched and logged rather than treated as fatal. A configured height is also no longer handed on to the code-driven fork list once it has been handled.
