...
- (payment, storage, challenge, sp, virtualgroup) [#402](https://github.com/mocachain/moca/pull/402) Compare account addresses by decoding them rather than by comparing their rendered form. Accounts are Ethereum accounts — twenty bytes of hex — so casing and the optional `0x` prefix carry no meaning, but comparisons across the modules used a mix of byte equality, case-insensitive string comparison and raw string equality, so whether two renderings of one account compared equal depended on the call site. `utils.SameAddress` and `utils.SameAddressOrEmpty` settle it, and the places that wrote a message address straight into state or an event now store the canonical rendering. `DisableRefund` compares the account's recorded owner directly rather than asking whether the sender owns the account, which additionally treats an account as its own owner. Address-derived store keys were already built from the decoded bytes and are unaffected.
