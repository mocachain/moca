...
- (app) [#391](https://github.com/mocachain/moca/pull/391) Apply a hardfork configured in `app.toml` in the block its height names, and stop `BeginBlock` from panicking when one coincides with an upgrade plan that is already set. Scheduled forks now run in `PreBlocker`: the upgrade module applies a due plan from its own `PreBlock` (moved there from `BeginBlock` by cosmos-sdk#17421), so a plan written in `BeginBlock` was too late for the block it named and only took effect in the following one. A configured hardfork now also takes precedence over an existing plan rather than panicking: a plan still waiting to be applied is canceled first, by executing the upgrade module's own `MsgCancelUpgrade`, so the cancellation is recorded in the block rather than the plan silently changing; a plan whose height has already passed is replaced; and a plan that cannot be read is left untouched and logged.
