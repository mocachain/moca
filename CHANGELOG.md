...
- (upgrade) [#263](https://github.com/mocachain/moca/pull/263) `v1.3.0` upgrade re-grants the authz grants the moca-iavl commit-time bug dropped from the merkle tree that moca's custom handlers require: each validator's self-del account → gov (`MsgDelegate`, required by `CheckStakeAuthorization` on create-validator) and each SP's funding account → gov (`MsgDeposit`, required by `CheckDepositAuthorization`). It is keyed off the canonical staking/sp stores, so the writes are identical on every node regardless of per-node authz fastnode drift, and `SaveGrant`'s `Set` re-adds each grant to the tree and overwrites any stale fastnode entry for that key. The handler does not purge the store: a 2-node test showed a purge keyed off the store iterator (which reads the fastnode, not the tree) diverges the apphash on a node whose fastnode is missing a tree-backed key, so the remaining fastnode drift is left to an IAVL rebuild (state-sync or a `fastStorageVersionValue` bump), which rebuilds the fastnode from the canonical tree and covers both drift directions without consensus risk
