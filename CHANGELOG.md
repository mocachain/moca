...
- (e2e) [#375](https://github.com/mocachain/moca/pull/375) Replace the dead, EIP-712-signing `e2e/tests` suite with an EVM/precompile-driven harness: every scenario now goes through its precompile over a real signed EVM transaction against a live node. 32 tests across storage, storageprovider, payment, and virtualgroup (29 passing, 3 documented skips for a pre-existing transient-storage-key bug and a GVG-deletion precondition gap)
