...
- (app) [#390](https://github.com/mocachain/moca/pull/390) Take hardfork heights from the binary on mainnet, and only honour the `app.toml` `[hardforks]` section on localnet, devnet and testnet — the chains its documentation names. Scheduling an upgrade writes a plan into consensus state, and `app.toml` is per-node with nothing reconciling it across a validator set, so mainnet now uses the code-driven path where every node running the same release derives the same heights. Also stop `BeginBlock` from panicking when a configured hardfork coincides with an upgrade plan that is already set: the existing plan, having been agreed on-chain, is kept and the configured entry is logged and skipped.
