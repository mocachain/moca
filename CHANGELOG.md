...
- (precompiles) [#356](https://github.com/mocachain/moca/pull/356) Converge the storage precompile onto the cosmos/evm precompile pattern (same structure as the other conversions) — the largest of the set: the full bucket/object/group/policy transaction and query set dispatched via `cmn.SetupABI` + `RunNativeAction` → `Execute` + `IsTransaction` (replacing the handler-registry map), split `contract.go` into `storage`/`tx`/`query`/`types`/`events`, inject the storage `MsgServer` while the keeper serves queries, and adopt cevm's flat+per-byte `RequiredGas` (dropping all per-message dynamic gas). moca-specifics preserved: hex (`0x`) addresses, the implicit-caller convention, per-message `ValidateBasic` plus the precompile-level argument checks (moved into the handlers), the ERC-721 bucket/object/group NFT `Transfer` events emitted to the token contracts via `AddOtherLog`, and the non-payable `RejectValue` + EOA-only guards. ABI and selectors are unchanged (`IStorage.go` untouched)
- (precompiles) [#356](https://github.com/mocachain/moca/pull/356) Converge the virtualgroup precompile onto the cosmos/evm precompile pattern (same structure as the other conversions): the ten global-virtual-group / SP-exit / swap transactions plus the family queries dispatched via `cmn.SetupABI` + `RunNativeAction` → `Execute` + `IsTransaction`, split `contract.go` into `virtualgroup`/`tx`/`query`/`types`/`events`, inject the virtualgroup `MsgServer` while the keeper serves queries, and adopt cevm's flat+per-byte `RequiredGas` (dropping the per-message dynamic gas). moca-specifics preserved: hex (`0x`) addresses, the implicit-caller convention, the per-message `ValidateBasic`, the `Approval` sub-struct handling, and the non-payable `RejectValue` + EOA-only guards. ABI and selectors are unchanged (`IVirtualGroup.go` untouched)
- (precompiles) [#356](https://github.com/mocachain/moca/pull/356) Converge the payment precompile onto the cosmos/evm precompile pattern (same structure as the other conversions): the create/deposit/disableRefund/withdraw transactions plus the payment-account, stream-record and params queries dispatched via `cmn.SetupABI` + `RunNativeAction` → `Execute` + `IsTransaction` (replacing the handler-registry map), split `contract.go` into `payment`/`tx`/`query`/`types`/`events`, inject the payment `MsgServer` while the keeper serves queries, and adopt cevm's flat+per-byte `RequiredGas`. moca-specifics preserved: hex (`0x`) addresses, the implicit-caller convention, the per-message `ValidateBasic`, and the non-payable `RejectValue` + EOA-only guards. ABI and selectors are unchanged (`IPayment.go` untouched)
- (precompiles) [#356](https://github.com/mocachain/moca/pull/356) Converge the authz precompile onto the cosmos/evm precompile pattern (same structure as the other conversions): `grant`/`revoke`/`exec` plus the grants, granterGrants and granteeGrants queries dispatched via `cmn.SetupABI` + `RunNativeAction` → `Execute` + `IsTransaction`, split `contract.go` into `authz`/`tx`/`query`/`types`/`events`, and adopt cevm's flat+per-byte `RequiredGas`. moca-specifics preserved verbatim: the `allowed:`/`denied:` authorization parsing (send/staking/sp-deposit authorization types), the `exec` message codec that registers the moca module interfaces, hex (`0x`) addresses, the implicit-caller convention, and the non-payable `RejectValue` + EOA-only guards. ABI and selectors are unchanged (`IAuthz.go` untouched)
- (precompiles) [#356](https://github.com/mocachain/moca/pull/356) Converge the storageprovider precompile onto the cosmos/evm precompile pattern (same structure as the other conversions): `updateSPPrice` plus the storage-provider and storage-price queries dispatched via `cmn.SetupABI` + `RunNativeAction` → `Execute` + `IsTransaction`, split `contract.go` into `storageprovider`/`tx`/`query`/`types`/`events`, inject the SP `MsgServer` while the keeper serves queries, and adopt cevm's flat+per-byte `RequiredGas`. moca-specifics preserved: hex (`0x`) addresses, the implicit-caller convention, and the non-payable `RejectValue` + EOA-only guards. ABI and selectors are unchanged (`IStorageProvider.go` untouched)
- (precompiles) [#356](https://github.com/mocachain/moca/pull/356) Converge the permission precompile onto the cosmos/evm precompile pattern (same structure as the other conversions): the `params` query dispatched via `cmn.SetupABI` + `RunNativeAction` → `Execute`, `contract.go` replaced by `permission`/`query`/`types`, inject the permission `QueryServer`, and adopt cevm's flat+per-byte `RequiredGas`. The module has no transaction methods (they were removed) and emits no events. moca-specifics preserved: hex (`0x`) address encoding and the non-payable `RejectValue` guard. ABI and selectors are unchanged (`IPermission.go` untouched)
- (precompiles) [#356](https://github.com/mocachain/moca/pull/356) Converge the staking precompile onto the cosmos/evm precompile pattern (same structure as the distribution/slashing/gov/bank conversions): `editValidator`/`delegate`/`undelegate`/`redelegate`/`cancelUnbondingDelegation` plus the delegation, validator, pool and params queries dispatched via `cmn.SetupABI` + `RunNativeAction` → `Execute` + `IsTransaction`, split `contract.go` into `staking`/`tx`/`query`/`types`/`events`, inject the staking `MsgServer`+`QueryServer`, and adopt cevm's flat+per-byte `RequiredGas`. moca-specifics preserved: hex (`0x`) addresses, the PoA/BLS `editValidator` path (`relayerAddress`/`challengerAddress`/`blsKey`/`blsProof` plus the commission-rate and min-self-delegation "unset" sentinels), the implicit-caller convention, and the non-payable `RejectValue` + EOA-only guards. ABI and selectors are unchanged (`IStaking.go` untouched)
- (precompiles) [#356](https://github.com/mocachain/moca/pull/356) Converge the bank precompile onto the cosmos/evm precompile pattern (same structure as the distribution/slashing/gov conversions): `send`/`multiSend` plus the balance, supply, metadata and send-enabled queries dispatched via `cmn.SetupABI` + `RunNativeAction` → `Execute` + `IsTransaction`, split `contract.go` into `bank`/`tx`/`query`/`types`/`events`, inject the bank `MsgServer` (built with moca's payment keeper so coin sends settle stream payments) while the bank keeper serves queries, and adopt cevm's flat+per-byte `RequiredGas`. moca-specifics preserved: hex (`0x`) address args and the non-payable `RejectValue` + EOA-only guards. ABI and selectors are unchanged (`IBank.go` untouched)
- (precompiles) [#356](https://github.com/mocachain/moca/pull/356) Converge the gov precompile onto the cosmos/evm precompile pattern (same structure as the distribution/slashing conversions): dispatch via `cmn.SetupABI` + `RunNativeAction` → `Execute` + `IsTransaction`, split `contract.go` into `gov`/`tx`/`query`/`types`/`events`, inject the gov `MsgServer`+`QueryServer` (plus the account keeper for the legacy-proposal authority), and adopt cevm's flat+per-byte `RequiredGas`. Proposal messages are now decoded and re-encoded through the application codec (a superset of the previous hand-rolled registry, so every registered message type is accepted). Additive ABI change: `cancelProposal(uint64)` and `constitution()` are added (regenerating `IGov.go`), while every existing method name, argument, and selector is preserved — existing clients are unaffected. moca-specifics preserved: hex (`0x`) address args, the implicit-caller convention, and the non-payable `RejectValue` + EOA-only guards
- (precompiles) [#356](https://github.com/mocachain/moca/pull/356) Converge the slashing precompile onto the cosmos/evm precompile pattern (same structure as the distribution conversion): `unjail` + `signingInfo`/`signingInfos`/`params` dispatched via `cmn.SetupABI` + `RunNativeAction` → `Execute` + `IsTransaction`, with an injected `MsgServer`+`QueryServer` and cevm's flat+per-byte `RequiredGas`. moca-specifics preserved: hex (`0x`) consensus/validator addresses, string-encoded `Params` fractions, and the non-payable `RejectValue` + EOA-only guards. ABI and selectors are unchanged (`ISlashing.go` untouched)
- (precompiles) [#356](https://github.com/mocachain/moca/pull/356) Converge the distribution precompile onto the cosmos/evm precompile pattern: dispatch via `cmn.SetupABI` + `RunNativeAction` → `Execute` + `IsTransaction`, split `contract.go` into `distribution`/`tx`/`query`/`types`/`events`/`errors`, inject the distribution `MsgServer`+`QueryServer`, and adopt cevm's flat+per-byte `RequiredGas`. moca-specifics preserved: hex (`0x`) address args, the `withdrawDelegatorAllRewards`/`params` method set, precision-preserving DecCoins, and the non-payable `RejectValue` + EOA-only guards. ABI and selectors are unchanged (`IDistribution.go` untouched), so there is no client impact
...
- (virtualgroup) [#396](https://github.com/mocachain/moca/pull/396) Compare the whole secondary SP list when `MsgCreateGlobalVirtualGroup` checks the family for an existing group with the same SPs in the same order. The comparison previously stopped once it had matched as many entries as the request held, so a stored group holding more secondaries than the request, but beginning with the same ones, was rejected as a duplicate and the new group could not be created. Groups of differing length arise within one family after the redundancy params change, since the required count is read from the versioned storage params. Nodes with and without this change can accept a different set of transactions, so it has to land in a coordinated upgrade.
- (permission) [#379](https://github.com/mocachain/moca/pull/379) Stop a bucket-level policy statement from silently voiding an owner's object `Deny` (MOCA-808). `PutPolicy` now rejects a bucket statement naming an object-scoped action (`GET_OBJECT`, `DELETE_OBJECT`, `COPY_OBJECT`, `EXECUTE_OBJECT`, `UPDATE_OBJECT_INFO`, `UPDATE_OBJECT_CONTENT`) with no `Resources`, and `Statement.Eval` now applies a `Resources`-less `Deny` to sub-resource access instead of discarding it. The evaluation change closes the same gap for `ACTION_TYPE_ALL` — which validation cannot reject without breaking legitimate bucket-wide policies — and for statements already stored before the validation landed. `Allow` is unchanged: a `Resources`-less `Allow` stays bucket-scoped, so no existing grant is widened.
...
- (precompiles) [#365](https://github.com/mocachain/moca/pull/365) Fix `virtualgroup.completeSPExit`'s `Operator` field, the one message #280/#362's sole-acting-identity audit found sourcing a signer field from caller-controlled calldata instead of `contract.Caller()`: `MsgCompleteStorageProviderExit.GetSigners()` returns `Operator` ahead of `StorageProvider` when set, so a caller could stamp an arbitrary address as their own exit's operator in the emitted `EventCompleteStorageProviderExit.OperatorAddress` and in the precompile's own EVM log topic. Not exploitable for authorization (the msgServer always authorizes on `StorageProvider == caller`). All 67 Cosmos messages constructed across the 11 transaction precompiles were audited against their actual `GetSigners()`; this was the only deviation.
...
- (deps) [#386](https://github.com/mocachain/moca/pull/386) Bump `cosmos/evm` v0.6.0 → v0.6.1, upstream's security patch release (state breaking): `x/vm` balance writes at statedb commit now reject module accounts (`"is not allowed to receive funds"`), closing a stale-balance mint/burn reconciliation vector against module pools; vesting accounts' EVM-denom `LockedCoins` are snapshotted at account load so commit-time bank-balance reconstruction survives mid-tx `DelegatedVesting` mutations (e.g. by the staking precompile); precompile out-of-gas now propagates `vm.ErrOutOfGas` instead of a revert; erc20 IBC ack validation hardened (dormant in moca — `x/erc20` stubbed, no IBC wired). Drop-in: no moca code changes (the new `BankKeeper.LockedCoins` requirement is satisfied by the SDK bank keeper, and all 11 precompiles inherit the gas fix via `cmn.Precompile.RunNativeAction` — a bank-precompile guard test pins the new out-of-gas semantics and fails on v0.6.0); the proto-download pin moves to v0.6.1 (no proto changes upstream)
...
- (virtualgroup) [#399](https://github.com/mocachain/moca/pull/399) Reject a new global virtual group once its family is already at `MaxGlobalVirtualGroupNumPerFamily`. The limit was compared against the family size before the new group was appended and only rejected past the maximum, so a family could hold one more than configured — eleven at the default limit of ten.
- (app) [#398](https://github.com/mocachain/moca/pull/398) Normalize the precompile blocklist keys to the form the bank keeper looks up. The list was built from the lowercase hex address literals while `BlockedAddr` looks entries up by `AccAddress.String()`, which is EIP-55 checksummed, so a literal whose checksum uppercases a character never matched. Of the sixteen listed precompiles this affected two — payment (`...100a`) and the deprecated erc20 address (`...100c`) — leaving the payment precompile able to receive funds; the nine Berlin precompiles are added already checksummed and were unaffected.
- (sp) [#397](https://github.com/mocachain/moca/pull/397) Delete the BLS-key index entry when a storage provider exits. `Keeper.Exit` passed the already-built index key back through `GetStorageProviderByBlsKeyKey`, so the delete targeted a key that is never written and the real entry outlived the provider it pointed at. Nothing reads back incorrectly — `GetStorageProviderByBlsKey` resolves the index and then loads the provider record, which `Exit` does delete, so lookups still report not-found — but one orphaned key was left behind per exited provider.
- (virtualgroup) [#388](https://github.com/mocachain/moca/pull/388) Settle the global virtual groups named in `MsgSettle` in a deterministic order. The handler deduplicated the requested ids into a map and then ranged that map, so when one of the ids could not be resolved, the number of groups settled before the error — and therefore the gas the message consumed — varied with Go's map iteration order. It now walks the request's own id list, which is the same on every node, and uses the map only to skip duplicates.
- (virtualgroup) [#372](https://github.com/mocachain/moca/pull/372) Restrict `MsgDeleteGlobalVirtualGroup` to the GVG's own primary SP, so an empty GVG can only be deleted — and its deposit refunded — by the storage provider that owns it.
