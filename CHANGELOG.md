...
- (app) [#390](https://github.com/mocachain/moca/pull/390) Stop `BeginBlock` from panicking when a hardfork configured in `app.toml` coincides with an upgrade plan that is already set. Governance can schedule a plan for a height an operator has also configured, and the panic left a node with nothing to recover to. The plan agreed on-chain is kept, the configured entry is logged and skipped, and a plan that cannot be read or scheduled is logged rather than treated as fatal. Successful scheduling is logged too.
