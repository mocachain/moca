...
- (upgrade) [#263](https://github.com/mocachain/moca/pull/263) `v1.3.0` upgrade resets the authz store to clear the fastnode-vs-merkle-tree drift the moca-iavl commit-time bug left there (some grants live in fastnode but were dropped from the merkle tree, so `prove=true` queries panic and state-synced nodes never received them — including the validator→gov delegate grants that `CheckStakeAuthorization` requires for create-validator). The handler purges every grant, then re-grants only the ones moca's custom handlers require — each validator's self-del account → gov (`MsgDelegate`) and each SP's funding account → gov (`MsgDeposit`), keyed off the canonical staking/sp stores so the result is deterministic on every node. Non-essential grants (user↔user, capped deposit authorizations) are dropped; owners re-create them
