...
### Bug Fixes
- (x/storage) [#450](https://github.com/mocachain/moca/pull/450) Backport to `release/1.4.x`: in `CopyObject`, verify the operator's create permission on the destination bucket, reject a destination object name that already exists, and set the copied object's owner to the destination bucket owner — matching `CreateObject` (MOCA-1421)
