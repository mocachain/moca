...
- (challenge) [#400](https://github.com/mocachain/moca/pull/400) Slash the remaining allowance instead of nothing once a storage provider is at `SpSlashMaxAmount`. A further slash in the counting window was set to zero rather than reduced to what was left under the cap, so a provider at the cap was slashed nothing until the window rolled over. The reduced amount is floored at zero because the first slash in a window is exempt from the cap and can already have taken the running total past it. Also corrects `KeySpSlashCountingWindow`, which carried the same literal as `KeySpSlashMaxAmount`; both constants are unreferenced, so this is a correction rather than a behaviour change.
