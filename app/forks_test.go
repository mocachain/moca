package app

import (
	"context"
	"strconv"
	"testing"

	"cosmossdk.io/core/header"
	"cosmossdk.io/x/upgrade"
	upgradetypes "cosmossdk.io/x/upgrade/types"
	sdk "github.com/cosmos/cosmos-sdk/types"
	"github.com/cosmos/cosmos-sdk/types/module"
	"github.com/stretchr/testify/require"

	servercfg "github.com/evmos/evmos/v12/server/config"
)

const forkTestHeight = 100

// withHardfork points the app at a node-local hardfork entry for the given height.
func withHardfork(mocaApp *Evmos, height int64, name string) {
	if mocaApp.appConfig == nil {
		mocaApp.appConfig = &servercfg.AppConfig{}
	}
	mocaApp.appConfig.Hardforks = map[string]servercfg.HardforkEntry{
		strconv.FormatInt(height, 10): {Name: name},
	}
}

// forkCtx builds a context at the given height. HeaderInfo has to be set as well
// as the block height: Plan.ShouldExecute reads HeaderInfo, so a context with only
// the block height set makes a due plan look pending.
func forkCtx(mocaApp *Evmos, height int64) sdk.Context {
	return mocaApp.NewContext(false).
		WithChainID("moca_5151-1").
		WithBlockHeight(height).
		WithHeaderInfo(header.Info{Height: height})
}

// A configured entry at the current height schedules the plan.
func TestScheduleForkUpgrade_ConfiguredHardforkSchedules(t *testing.T) {
	mocaApp := EthSetup(false, nil)
	withHardfork(mocaApp, forkTestHeight, "test-hardfork")

	ctx := forkCtx(mocaApp, forkTestHeight)

	mocaApp.ScheduleForkUpgrade(ctx)

	plan, err := mocaApp.UpgradeKeeper.GetUpgradePlan(ctx)
	require.NoError(t, err)
	require.Equal(t, "test-hardfork", plan.Name)
	require.Equal(t, int64(forkTestHeight), plan.Height)
}

// A hardfork is the path for an emergency that cannot wait on a proposal, so it
// supersedes an upgrade that is only pending rather than being skipped. This runs
// in BeginBlock, so the disagreement must not stop the chain either.
func TestScheduleForkUpgrade_SupersedesPendingPlan(t *testing.T) {
	mocaApp := EthSetup(false, nil)
	withHardfork(mocaApp, forkTestHeight, "config-hardfork")

	ctx := forkCtx(mocaApp, forkTestHeight)

	// Pending: scheduled for a height the chain has not reached.
	require.NoError(t, mocaApp.UpgradeKeeper.ScheduleUpgrade(ctx,
		upgradetypes.Plan{Name: "gov-upgrade", Height: forkTestHeight + 50}))

	require.NotPanics(t, func() { mocaApp.ScheduleForkUpgrade(ctx) })

	plan, err := mocaApp.UpgradeKeeper.GetUpgradePlan(ctx)
	require.NoError(t, err)
	require.Equal(t, "config-hardfork", plan.Name, "the hardfork must take precedence")
	require.Equal(t, int64(forkTestHeight), plan.Height)

	// The cancellation has to be announced: nothing following the chain can see
	// a plan that was only overwritten.
	require.True(t, hasCancelUpgradeEvent(ctx),
		"canceling the pending plan must be visible in the block's events")
}

// hasCancelUpgradeEvent reports whether a cancel-upgrade message was executed,
// which is the same event baseapp emits for one submitted in a transaction.
func hasCancelUpgradeEvent(ctx sdk.Context) bool {
	want := sdk.MsgTypeURL(&upgradetypes.MsgCancelUpgrade{})
	for _, ev := range ctx.EventManager().Events() {
		if ev.Type != sdk.EventTypeMessage {
			continue
		}
		for _, a := range ev.Attributes {
			if a.Key == sdk.AttributeKeyAction && a.Value == want {
				return true
			}
		}
	}
	return false
}

// A plan due at this very height has not been applied yet either, because this
// runs ahead of the upgrade module, so it is canceled rather than treated as
// already spent.
func TestScheduleForkUpgrade_SupersedesPlanDueThisBlock(t *testing.T) {
	mocaApp := EthSetup(false, nil)
	withHardfork(mocaApp, forkTestHeight, "config-hardfork")

	ctx := forkCtx(mocaApp, forkTestHeight)
	require.NoError(t, mocaApp.UpgradeKeeper.ScheduleUpgrade(ctx,
		upgradetypes.Plan{Name: "gov-upgrade", Height: forkTestHeight}))

	require.NotPanics(t, func() { mocaApp.ScheduleForkUpgrade(ctx) })

	plan, err := mocaApp.UpgradeKeeper.GetUpgradePlan(ctx)
	require.NoError(t, err)
	require.Equal(t, "config-hardfork", plan.Name)
	require.True(t, hasCancelUpgradeEvent(ctx))
}

// A plan at or before the current height cannot still be waiting to be applied,
// so it is replaced rather than treated as a conflict.
func TestScheduleForkUpgrade_ReplacesStalePlan(t *testing.T) {
	mocaApp := EthSetup(false, nil)

	earlier := forkCtx(mocaApp, forkTestHeight)
	require.NoError(t, mocaApp.UpgradeKeeper.ScheduleUpgrade(earlier,
		upgradetypes.Plan{Name: "old-upgrade", Height: forkTestHeight}))

	later := int64(forkTestHeight + 50)
	withHardfork(mocaApp, later, "config-hardfork")
	ctx := forkCtx(mocaApp, later)

	require.NotPanics(t, func() { mocaApp.ScheduleForkUpgrade(ctx) })

	plan, err := mocaApp.UpgradeKeeper.GetUpgradePlan(ctx)
	require.NoError(t, err)
	require.Equal(t, "config-hardfork", plan.Name)
	require.Equal(t, later, plan.Height)
}

// Re-running the same height must be a no-op rather than a second write.
func TestScheduleForkUpgrade_AlreadyScheduledIsIdempotent(t *testing.T) {
	mocaApp := EthSetup(false, nil)
	withHardfork(mocaApp, forkTestHeight, "test-hardfork")

	ctx := forkCtx(mocaApp, forkTestHeight)

	mocaApp.ScheduleForkUpgrade(ctx)
	require.NotPanics(t, func() { mocaApp.ScheduleForkUpgrade(ctx) })

	plan, err := mocaApp.UpgradeKeeper.GetUpgradePlan(ctx)
	require.NoError(t, err)
	require.Equal(t, "test-hardfork", plan.Name)
	require.Equal(t, int64(forkTestHeight), plan.Height)
}

// A configured height is handled by configuration whatever the outcome, so the
// caller must not go on to consult the code-driven fork list. That list is empty
// today, and an entry added to it at a configured height would otherwise be
// scheduled on top of the hardfork.
func TestScheduleConfiguredHardfork_ConfiguredHeightIsClaimed(t *testing.T) {
	mocaApp := EthSetup(false, nil)
	withHardfork(mocaApp, forkTestHeight, "config-hardfork")

	ctx := forkCtx(mocaApp, forkTestHeight)
	require.True(t, mocaApp.scheduleConfiguredHardfork(ctx))

	// And a height with no entry hands back to the code-driven list.
	other := forkCtx(mocaApp, forkTestHeight+1)
	require.False(t, mocaApp.scheduleConfiguredHardfork(other))
}

// The fork must take effect in the block its height names. The upgrade module
// applies a due plan from its own PreBlock, so scheduling has to happen ahead of
// it in the same block; scheduling in BeginBlock left the plan sitting in state
// until the following block. Registering a handler lets the plan be applied
// rather than halting, so a completed upgrade proves the timing.
//
// The two steps are called directly rather than through app.PreBlocker, which
// also runs the EVM module's PreBlock: that seals process-global config and
// panics once another test in this package has already sealed it.
func TestPreBlocker_ConfiguredHardforkAppliesInItsOwnBlock(t *testing.T) {
	mocaApp := EthSetup(false, nil)
	withHardfork(mocaApp, forkTestHeight, "same-block-hardfork")

	applied := false
	mocaApp.UpgradeKeeper.SetUpgradeHandler("same-block-hardfork",
		func(_ context.Context, _ upgradetypes.Plan, vm module.VersionMap) (module.VersionMap, error) {
			applied = true
			return vm, nil
		})

	ctx := forkCtx(mocaApp, forkTestHeight)

	// Same order as PreBlocker: schedule, then let the upgrade module act.
	mocaApp.ScheduleForkUpgrade(ctx)
	_, err := upgrade.PreBlocker(ctx, mocaApp.UpgradeKeeper)
	require.NoError(t, err)

	require.True(t, applied, "the handler must run in the block the fork height names")

	// Applying clears the plan and records the height it completed at.
	_, err = mocaApp.UpgradeKeeper.GetUpgradePlan(ctx)
	require.ErrorIs(t, err, upgradetypes.ErrNoUpgradePlanFound)
	done, err := mocaApp.UpgradeKeeper.GetDoneHeight(ctx, "same-block-hardfork")
	require.NoError(t, err)
	require.Equal(t, int64(forkTestHeight), done)
}
